...
class processo_filterTemporal(object):
    ...
    options = {
            'output_asset': 'projects/mapbiomas-workspace/AMOSTRAS/col8/CAATINGA/POS-CLASS/Temporal/',
    ...
    def __init__(self):
        ...
        self.ordem_exec_first = [4,3,12,21] #
        self.ordem_exec_last = [4,3,12,21]  # 29
        self.ordem_exec_middle = [4,3,12,21,33] # ,33, 22,      
    ...
    def applyTemporalFilter(self, id_bacias, iteracao): 
        self.colectAnos = [self.mapeiaAnos(
                                ano, self.options['janela'], self.years) for ano in self.years]       
        # clase = {
        #         '21': "agro",
        ...
        # name_imgClass = 'filterFQ_BACIA_'+ id_bacias  + "_V" + self.versionFR 
        name_imgClass = 'filterSP_BACIA_'+ id_bacias  + "_V" + self.versionFR 
        # name_imgClass = 'filterTP_BACIA_'+ id_bacias  + "_V" + self.versionFR 
        imgClass = ee.Image(self.options['input_asset'] + name_imgClass)  
        if iteracao > 1:
            for itera in range(iteracao):
                print("processing map na interação # ", itera)
                self.options['janela'] = 3
        ...
        name_toexport = 'filterTP_BACIA_'+ str(id_bacias) + "_V" + self.versionTP     
...
listaNameBacias = [
    ...
    # '767','771','772','773','775','776','777','778','76111',
    # '76116','7612','7614','7615','7616','7617','7618','7619', 
    # '7613','754','756','757','763',
    '7741','7742'
]
...
# ]
# listaNameBacias = ["7616","7742","7741"]
...
for cc, idbacia in enumerate(listaNameBacias[:]):   
    if knowMapSaved:
        try:
            ...
            nameMap = 'filterTP_BACIA_' + idbacia + "_V12"
            print(input_asset + nameMap)
            imgtmp = ee.Image(input_asset + nameMap)
            print(cc, " loading ", nameMap, " ", len(imgtmp.bandNames().getInfo()), "bandas ")
            # if cc == 0:
        ...
        iterando = 1
        aplicando_TemporalFilter.applyTemporalFilter(idbacia, iterando)
