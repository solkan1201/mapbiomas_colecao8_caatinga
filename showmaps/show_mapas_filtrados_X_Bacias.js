...
var param = { 
    ...
    asset_filted : {
        ...
        'stavel':'projects/mapbiomas-workspace/AMOSTRAS/col8/CAATINGA/POS-CLASS/Spatial',
        'spatial':'projects/mapbiomas-workspace/AMOSTRAS/col8/CAATINGA/POS-CLASS/SSpatial',
...
var imgMapCol8SpaV4 =  ee.ImageCollection(param.asset_filted.spatial)
                                    .filter(ee.Filter.eq('version', '4'))
                                    .filter(ee.Filter.eq('id_bacia', selBacia)).first();
imgMapCol8SpaV4 = imgMapCol8SpaV4.select(banda_activa)//.remap(param.classMapB, param.classNew);

var imgMapCol8TempV7 =  ee.ImageCollection(param.asset_filted.temporal)
                                    .filter(ee.Filter.eq('version', '7'))
                                    .filter(ee.Filter.eq('id_bacia', selBacia)).first();
imgMapCol8TempV7 = imgMapCol8TempV7.select(banda_activa)//.remap(param.classMapB, param.classNew);
...
Map.addLayer(imgMapCol8SpaV4,  visualizar.visclassCC, 'Col8_Spacial V2<-5', false);
Map.addLayer(imgMapCol8TempV7,  visualizar.visclassCC, 'Col8_Temporal V2<-10', false);
